...
bool32 bigOpenAllBigFiles(void)
{
    ...
    for (bigfile_i = 0; bigfile_i < NUMBER_CONFIGURED_BIG_FILES; ++bigfile_i)
    {
        ...
        if (bigFilePrecedence[bigfile_i].filePtr != NULL)
        {
            ...
            // TEMP: dump TOC entries containing "arden" or "level" for path debugging
            {
                char tocName[BF_MAX_FILENAME_LENGTH + 1];
                int tocDumpI;
                bigTOC *dumpTOC = &bigFilePrecedence[bigfile_i].tableOfContents;
                for (tocDumpI = 0; tocDumpI < dumpTOC->numFiles; tocDumpI++)
                {
                    bigFilenameRetrieve(dumpTOC, tocDumpI, bigFilePrecedence[bigfile_i].filePtr, tocName);
                    if (strstr(tocName, "arden") || strstr(tocName, "level") || strstr(tocName, "Level"))
                        dbgMessagef("TOC[%d]: %s", tocDumpI, tocName);
                }
            }
