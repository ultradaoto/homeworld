...
void aiplayerChooseEnemies(udword num_players, udword num_human, udword num_comp)
{
    ...
    if (num_players != num_human + num_comp)
    {
        aiplayerLog((0,"Warning: aiplayerChooseEnemies player count mismatch (%d != %d+%d), skipping",
                     num_players, num_human, num_comp));
        return;
    }
...
void FindEnemies(AIPlayer *aiplayer)
{
    ...
    aiplayer->enemyPlayerCount = (universe.numPlayers > 0) ? universe.numPlayers - 1 : 0;
