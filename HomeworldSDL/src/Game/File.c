...
#ifdef _WIN32
    /* fileNameReducePath rebuilds with forward slashes; normalise to backslashes
       so the strchr('\\') path-splitting logic below works correctly. */
    for (size_t _si = 0; _si < directoryLen; _si++)
        if (directoryCopy[_si] == '/') directoryCopy[_si] = '\\';
#endif
...
sdword fileLoadAlloc(char *_fileName, void **address, udword flags)
{
    ...
    {
        dbgMessagef("fileLoadAlloc: called with empty/null filename (flags=0x%x), returning -1", flags);
    ...
    if ((sdword)length <= 0)                               //fileSizeGet returns -1 for missing/bad paths
    {
        dbgMessagef("fileLoadAlloc: skipping '%s' (fileSizeGet returned %d)", fileName, (sdword)length);
        *address = NULL;
        return -1;
    }
...
sdword fileSizeGet(char *_fileName, udword flags)
{
    ...
    if (_fileName == NULL || _fileName[0] == '\0')
    {
        dbgMessagef("fileSizeGet: called with empty/null filename (flags=0x%x), returning -1", flags);
        return -1;
    }
    ...
    {
        dbgMessagef("fileSizeGet: can't find file '%s' (returning -1)", fileName);
        return -1;
    }
