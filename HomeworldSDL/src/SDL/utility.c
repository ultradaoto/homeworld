...
*/
    ...
    else if (!singlePlayerGame)
    {
        if (loadfilename == NULL)
        {
            dbgMessagef("DEBUG gameStart: useScenarioFile='%s' numPlayers=%d multiPlayerGame=%d",
                        useScenarioFile ? useScenarioFile : "<NULL>", numPlayers, multiPlayerGame);
            if (useScenarioFile == NULL || useScenarioFile[0] == '\0')
            {
                dbgMessagef("DEBUG gameStart: useScenarioFile is empty/NULL — aborting load");
                return;
            }
            sprintf(levelfile,useScenarioFile,numPlayers);
            dbgMessagef("DEBUG gameStart: levelfile='%s'", levelfile);
...
void utyNewGameStart(char *name, featom *atom)
{
    ...
    if (!multiPlayerGame)       // moved up so that check if minplayers too low can be made before feAllScreensDelete, etc
    {
        ...
        dbgMessagef("DEBUG utyNewGameStart: multiPlayerGame=%d singlePlayerGame=%d numPlayers=%d spCurrentSelected=%d",
                    multiPlayerGame, singlePlayerGame, numPlayers, spCurrentSelected);
        dbgMessagef("DEBUG utyNewGameStart: spScenarios[%d].fileSpec='%s'",
                    spCurrentSelected,
                    spScenarios[spCurrentSelected].fileSpec ? spScenarios[spCurrentSelected].fileSpec : "<NULL>");
        dbgMessagef("DEBUG utyNewGameStart: tpGameCreated.MapName='%s'", tpGameCreated.MapName);
