...
lifheader *trLIFFileLoad(char *fileName, udword flags)
{
    ...
    {
        dbgMessagef("trLIFFileLoad: called with empty/null filename, returning NULL");
        return NULL;
    }
    ...
    if (oldHeader == NULL || loadLength <= 0)
    {
        dbgMessagef("trLIFFileLoad: fileLoadAlloc failed for '%s' (flags=0x%x), returning NULL", fileName, flags);
        return NULL;
    }
...
void trMeshSortListLoad(trmeshsort *sortList)
{
    ...
    for (index = 0; index < sortList->nTextures; index++)
    {
        ...
        {                                                   //if texture needs loading
            ...
            dbgMessagef("DEBUG trRegistryRefresh: loading texture '%s' (reg->fileName='%s')", fullName, reg->fileName);
            lifFile = trLIFFileLoad(fullName, 0);           //load in the file
            if (lifFile == NULL)
            {
                dbgMessagef("trRegistryRefresh: skipping missing texture '%s'", fullName);
                trClearPending(sortList->textureList[index]);
                continue;
            }
